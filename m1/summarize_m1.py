...
import sys, os, json
rows = []
def fmt(v, scale, nd): return f"{scale * v:.{nd}f}" if isinstance(v, (int, float)) else "—"
for d in sys.argv[1:]:
    ...
    rows.append(dict(run=os.path.basename(d), window=r["window"], frames=f"{r['frames_registered']}/{r['frames_total']}", cells=r["cells"], dfrac=f"{100 * r.get('d_estimated_fraction', float('nan')):.0f} %",
                     ...
                     ev=(f"z={e['z_mm']:.0f}: truth {100 * e['gt_reduction_seen']:.0f} %, M1 {fmt(e.get('est_smoothed_reduction', e.get('est_model_reduction')), 100, 0)} %; "
                         f"RMSE M1 {fmt(e.get('rmse_smoothed', e.get('rmse_model')), 1, 1)} vs rigid {fmt(e.get('rmse_rigid'), 1, 1)} mm²; event-window coverage {e.get('event_window_frames_estimated', '?')}/{e.get('event_window_frames_seen', '?')}") if e else "—"))
