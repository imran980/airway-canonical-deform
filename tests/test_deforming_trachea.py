...
def test_cartilage_sector_is_rigid(tmp_path):
    ...
    assert np.abs(g["D"][:, :, cart <= 0.5]).max() > 0.5, "membrane must move"
...
def test_collapse_is_local_in_time_and_space(tmp_path):
    ...
    assert CSA[it, iz] < 0.35 * CSA[0, iz], "a near-occlusive event (>=65% area loss)"
    far = int(np.argmin(np.abs(z - (p.collapse_z0_mm - 4 * p.collapse_sigma_z_mm))))
    if 0 <= far < len(z):
        drop_far = 1 - CSA[it, far] / CSA[0, far]; assert drop_far < 0.3, "far from z0 only breathing remains"
...
def test_gt_file_roundtrip(tmp_path):
    ...
    assert f["csa_mm2"].shape == g["CSA"].shape and abs(json.loads(str(f["params"]))["breath_amp_mm"] - p.breath_amp_mm) < 1e-9
