...
MINF = int(sys.argv[sys.argv.index("--min-frames") + 1]) if "--min-frames" in sys.argv else 60
runs = [x for x in sys.argv[1:] if not x.startswith("--") and not x.isdigit()]
def ts(a, v):
...
for run in runs:
    ...
    for i in range(dev.shape[1]):
        ...
        if ok.sum() < MINF or np.std(dist[ok]) < 0.3: continue
    ...
    if len(sl) == 0: print(f"[slope] {run}: no station with >= {MINF} frames (cells {int(np.isfinite(ratio).sum())})"); continue
